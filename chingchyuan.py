...
class CHINGCHYUAN():
            ...
            if self._PDFData(currentPage) and self.window.RunStatus:
                for self.idx in range(self.currentNum-1,self.datalen) :
                    ...
                    if ((currentPage != self.EndPage) and (self.idx != self.EndNum)) and self.window.RunStatus:
                        ...
                        self.log.write(self.Data[self.idx]['Name'],self.Data[self.idx]['ID'],"清泉醫院",self.Data[self.idx]['Born'],str(currentPage + 1),str(self.idx + 1))
                        time.sleep(2)
                    else:
                        break
            else:
    ...
    def _getReslut(self,name:str, ID:str, year:str, month:str, day:str):
        ...
        with httpx.Client(http2=True) as client :
            # 進入網頁
            self.respone = client.post('http://webreg.ching-chyuan.com.tw:81/OReg/GetPatInfo', data=self.payload, headers=self.headers)
            if(self.respone.json()[0]['msg'] == "病患不存在"):
                ...
                self.respone = client.get('http://webreg.ching-chyuan.com.tw:81/OReg/ScheduledRecords')
                self.respone2 = client.post('http://webreg.ching-chyuan.com.tw:81/OReg/GetEventsByCondition', data=self.payload2, headers=self.headers)
    ...
    def _changeHTMLStyle(self,page_content):
        ...
        for Tag in Tags:
            Tag['href'] = "http://webreg.ching-chyuan.com.tw:81/" + Tag['href']
        # 替換屬性內容，強制複寫資源路徑(針對img)
        Tags = soup.find_all(['img'])
        for Tag in Tags:
            Tag['src'] = "http://webreg.ching-chyuan.com.tw:81/Content/onlineregister/logo.png"
        #增加掛號table
