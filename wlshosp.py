...
class WLSHOSP():
            ...
            try:
                with httpx.Client(http2=True) as client :
                    ...
                    if(respone.json()[0]['msg'] == "病患不存在"):
                        ...
                        with open("reslut_烏日林新醫院.html", "w", encoding="utf-8") as f:
    ...
    def _startBrowser(self,name,ID):
        self.browser.get(r'file:///' + (os.path.dirname(os.path.abspath(__file__)).replace('\_internal','')) + '/reslut_烏日林新醫院.html')
    ...
    def _changeHTMLStyle(self,page_content):
        ...
        for Tag in Tags:
            Tag['href'] = "https://booking.wlshosp.org.tw/" + Tag['href']

        #增加掛號table
        Tags = soup.find(['table'])
        Tags['style'] = "width:100%;border:0px;font-size:2.0em;"
        return str(soup)
    ...
    def _JSONDataToHTML(self,jsonData,page_content):
        ...
        for i in range(len(datas)):
            ...
            new_td_tag.attrs = {"style":"text-align:center", "class":"col-xs-1 col-md-1"}
            new_td_tag.string = datas[i]['patname']
            target_tag[i+1].append(new_td_tag)

            new_td_tag = soup.new_tag("td")
            new_td_tag.attrs = {"style":"text-align:center", "class":"col-xs-3 col-md-2"}
            new_td_tag.string = datas[i]['datetime']
            target_tag[i+1].append(new_td_tag)

            new_td_tag = soup.new_tag("td")
            new_td_tag.attrs = {"style":"text-align:center", "class":"col-xs-2 col-md-2"}
